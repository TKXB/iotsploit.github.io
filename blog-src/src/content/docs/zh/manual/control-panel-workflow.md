...
本指南依据正式版源码（提交 `c3f20ff8`，版本 `0.0.17+17`）记录所有可见标签和行为。涵盖初始数据加载、目标和设备选择、驱动程序管理、插件参数提示、同步与异步执行、两个日志选项卡，以及应用停止、需要人工分析的边界。
...
控制面板不在本地运行插件。它向已配置的 API 服务器发送 HTTP 请求，并通过 WebSocket 连接到已配置的 WebSocket 服务器流式传输进度。应用本身不持有插件代码，执行由后端负责。

三个服务支撑该界面：

- **PluginService** — `GET /api/list_plugin_info/` 获取目录，`POST /api/execute_plugin/` 启动运行，`ws://<ws_base_url>/ws/exploit/<task_id>/` 获取异步进度。
- **TargetsService** — `GET /api/list_targets/` 获取清单，`POST /api/select_target/` 告知后端测试哪个目标。
- **TestResultService** — 将完成和失败的结果存储在设备上的 SharedPreferences 中，而非服务器上。键为 `plugin_test_results`。

系统日志面板是独立组件。它连接到 `ws://<ws_base_url>/ws/console_logs/` 获取实时后端日志，`GET /api/console_logs/` 作为回退。它与插件执行无关。
